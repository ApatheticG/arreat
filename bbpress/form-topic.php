	...
	<div id="new-topic-<?php bbp_topic_id(); ?>" class="bbp-topic-form">
		...
		<form id="new-post" name="new-post" method="post" action="<?php the_permalink(); ?>">
			...
			<fieldset class="bbp-form">
				...
				<div class="arreat-reply-form">

                    <div class="arreat-reply-form-left">
                        <?php bbp_current_user_avatar();?>
                        <div class="arreat-reply-form-name">
                            <?php bbp_user_profile_link(bbp_get_current_user_id()); ?>
                        </div>
                        <div class="arreat-reply-form-role">
                            <?php bbp_user_display_role(bbp_get_current_user_id());?>
                        </div>
                    </div>


                    <div class="arreat-reply-form-right">
                        <?php bbp_get_template_part( 'form', 'anonymous' ); ?>

                        <?php do_action( 'bbp_theme_before_topic_form_title' ); ?>
                        <p>
                            <input type="text" id="bbp_topic_title" class="arreat-topic-title-input" value="<?php bbp_form_topic_title(); ?>" tabindex="<?php bbp_tab_index(); ?>" size="40" name="bbp_topic_title" maxlength="<?php bbp_title_max_length(); ?>" placeholder="<?php printf( __( 'Topic Title (Maximum Length: %d):', 'bbpress' ), bbp_get_title_max_length() ); ?>"/>
                        </p>
                        <?php do_action( 'bbp_theme_after_topic_form_title' ); ?>

                        <?php do_action( 'bbp_theme_before_topic_form_content' ); ?>
                        <?php bbp_the_content( array( 'context' => 'topic', 'tinymce' => true, 'media_buttons' => true ) ); ?>
                        ...
                        <div class="arreat-reply-form-right-col-1">

                            <?php if ( !bbp_is_single_forum() ) : ?>
                                <?php do_action( 'bbp_theme_before_topic_form_forum' ); ?>
                                <p>
                                    <label for="bbp_forum_id"><?php _e( 'Forum:', 'bbpress' ); ?></label> <?php
                                        bbp_dropdown( array(
                            ...
                            <?php if ( current_user_can( 'moderate' ) ) : ?>
                                ...
                                <p>
                                    <label for="bbp_stick_topic"><?php _e( 'Topic Type:', 'bbpress' ); ?></label> <?php bbp_form_topic_type_dropdown(); ?>
                                </p>
                                <?php do_action( 'bbp_theme_after_topic_form_type' ); ?>
                                <?php do_action( 'bbp_theme_before_topic_form_status' ); ?>
                                <p>
                                    <label for="bbp_topic_status"><?php _e( 'Topic Status:', 'bbpress' ); ?></label> <?php bbp_form_topic_status_dropdown(); ?>
                                </p>
                        ...
                        <div class="arreat-reply-form-right-col-2">

                            <?php if ( bbp_allow_topic_tags() && current_user_can( 'assign_topic_tags' ) ) : ?>
                                <?php do_action( 'bbp_theme_before_topic_form_tags' ); ?>
                                <p>
                                    <input type="text" value="<?php bbp_form_topic_tags(); ?>" tabindex="<?php bbp_tab_index(); ?>" size="40" name="bbp_topic_tags" id="bbp_topic_tags" <?php disabled( bbp_is_topic_spam() ); ?> placeholder="<?php _e( 'Topic Tags:', 'bbpress' ); ?>"/>
                                </p>
                                ...
                                <p>
                                    <label for="bbp_topic_subscription">
                                        <input name="bbp_topic_subscription"
                                               id="bbp_topic_subscription"
                                               type="checkbox"
                                               value="bbp_subscribe"
                                               <?php bbp_form_topic_subscribed(); ?>
                                               <?php if (!bbp_is_topic_edit()) { echo 'checked="checked"';}?>
...
<?php else : ?>
    ...
    <?php if (function_exists('get_ulogin_panel')) {
