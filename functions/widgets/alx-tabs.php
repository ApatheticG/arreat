...
/* ------------------------------------ */
    private function _create_tabs($tabs,$count) {
        ...
        $titles = array(
            ...
            'tags'     => __('Tags','hueman'),
            'topics'   => __('Topics','hueman'),
            'replies'  => __('Replies','hueman')
        );
        $icons = array(
            'recent'   => 'fa fa-clock-o',
            'popular'  => 'fa fa-clock-o',
            'comments' => 'fa fa-comments-o',
            'tags'     => 'fa fa-tags',
            'topics'   => 'fa fa-clock-o',
            'replies'  => 'fa fa-commenting-o'
        );
...
/* ------------------------------------ */
    ...
    $order = array(
        ...
        'topics'   => $instance['order_topics'],
        'replies'  => $instance['order_replies']
    );
    asort($order);
...
?>
    ...
    <div class="alx-tabs-container">
        ...
        <?php if($instance['topics_enable']) { // Recent posts enabled? ?>
            ...
            switch ( $instance["topics_order"] ) {
                ...
                case 'freshness' :
                    $topics_query = array(
                        ...
                        'post_parent'         => $instance['topics_parent'],
                        'posts_per_page'      => (int) $instance["topics_num"],
            ...
            ?>
            <?php $topics=new WP_Query( $topics_query ); ?>
        ...
        <?php if($instance['replies_enable']) { // Recent posts enabled? ?>

            <?php
                $replies_query = array(
                    'post_type'           => bbp_get_reply_post_type(),
                    'post_status'         => array( bbp_get_public_status_id(), bbp_get_closed_status_id() ),
                    'posts_per_page'      => (int) $instance["replies_num"],
                    'ignore_sticky_posts' => true,
                    'no_found_rows'       => true,
                );
            ?>
            <?php $replies=new WP_Query( $replies_query ); ?>

            <ul id="tab-replies" class="alx-tab group">
                <?php while ($replies->have_posts()): $replies->the_post(); ?>
                <li>

                        <?php if($instance['replies_avatars']) { // Avatars enabled? ?>
                        <div class="tab-item-avatar">
                            <a href="">
                                <?php echo get_avatar($comment->comment_author_email,$size='96'); ?>
                            </a>
...
/* ------------------------------------ */
    ...
    // Recent comments
        $instance['topics_enable'] = $new['topics_enable']?1:0;
        $instance['topics_parent'] = strip_tags($new['topics_parent']);
        $instance['topics_num']    = strip_tags($new['topics_num']);
        $instance['topics_order']  = strip_tags($new['topics_order']);
    // Order
        ...
        $instance['order_topics']   = strip_tags($new['order_topics']);
        $instance['order_replies']  = strip_tags($new['order_replies']);
        return $instance;
    }
...
?>
    ...
    <div class="alx-options-tabs">
        ...
        <p>
            <input type="checkbox" class="checkbox" id="<?php echo esc_attr( $this->get_field_id('topics_enable') ); ?>" name="<?php echo esc_attr( $this->get_field_name('topics_enable') ); ?>" <?php checked( (bool) $instance["topics_enable"], true ); ?>>
            <label for="<?php echo esc_attr( $this->get_field_id('topics_enable') ); ?>">Включить темы</label>
        </p>

        <p>
            <label style="width: 55%; display: inline-block;" for="<?php echo esc_attr( $this->get_field_id("topics_num") ); ?>">Items to show</label>
            <input style="width:20%;" id="<?php echo esc_attr( $this->get_field_id("topics_num") ); ?>" name="<?php echo esc_attr( $this->get_field_name("topics_num") ); ?>" type="text" value="<?php echo absint($instance["topics_num"]); ?>" size='3' />
        </p>

        <p>
            <label for="<?php echo esc_attr( $this->get_field_id("topics_parent") ); ?>">ID Родительского Форума</label>
            <input class="widefat" id="<?php echo esc_attr( $this->get_field_id("topics_parent") ); ?>" name="<?php echo esc_attr( $this->get_field_name("topics_parent") ); ?>" type="text" value="<?php echo absint($instance["topics_parent"]); ?>" size='3' />
            <br>
            <small>"0" для отображения только корня - "any" для отображения всех</small>
        </p>

        <p style="padding-top: 0.3em;">
            <label style="width: 100%; display: inline-block;" for="<?php echo esc_attr( $this->get_field_id("topics_order") ); ?>">Сортировать по:</label>
            <select style="width: 100%;" id="<?php echo esc_attr( $this->get_field_id("topics_order") ); ?>" name="<?php echo esc_attr( $this->get_field_name("topics_order") ); ?>">
              <option value="freshness"<?php selected( $instance["topics_order"], "freshness" ); ?>>Темы с последними ответами</option>
              <option value="popular"<?php selected( $instance["topics_order"], "popular" ); ?>>Популярные темы</option>
              <option value="newness"<?php selected( $instance["topics_order"], "newness" ); ?>>Новые темы</option>
            </select>
        </p>
        ...
        <p>
            <label style="width: 55%; display: inline-block;" for="<?php echo esc_attr( $this->get_field_id("order_topics") ); ?>">Темы</label>
            <input class="widefat" style="width:20%;" type="text" id="<?php echo esc_attr( $this->get_field_id("order_topics") ); ?>" name="<?php echo esc_attr( $this->get_field_name("order_topics") ); ?>" value="<?php echo esc_attr( $instance["order_topics"] ); ?>" />
        </p>
        <p>
            <label style="width: 55%; display: inline-block;" for="<?php echo esc_attr( $this->get_field_id("order_reply") ); ?>">Ответы</label>
            <input class="widefat" style="width:20%;" type="text" id="<?php echo esc_attr( $this->get_field_id("order_reply") ); ?>" name="<?php echo esc_attr( $this->get_field_name("order_reply") ); ?>" value="<?php echo esc_attr( $instance["order_reply"] ); ?>" />
        </p>
